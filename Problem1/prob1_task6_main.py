...
def main():
    ...
    def check_traps(x: int, y: int, traps: set) -> int:
        """check_traps function that checks if a grid position is a trap

        Args:
            x (int): x axis value
            y (int): y axis value
            traps (set): a set that contains all traps
    ...
    repeats = 10000  # change to 100000
    density = 10 ** (-2)
    check_values = {0, 499}

    def traps_creator(density: float, seed: int) -> set:
        """traps_creator a function that generates a set of (x,y) traps
        based on the density argument's input. It creates rng integers which get added to the set if they're not in it already.
        Then it checks the size of the set and if it's smaller than asked, it breaks the loop and gets a new pair of random values.
        If it
        ...
        """
        answer: set = set()
        size: int = round(density * 500 * 500)
    ...
    def random_walk_step(density: int):
        trap_time = np.zeros((repeats))
        for i in trange(repeats):
            time = 0
            # grid = np.zeros((500, 500), dtype=np.int8)
            np.random.seed(4397 + i)
            traps = traps_creator(density, 4397 + i)
            trap_time: np.ndarray = np.zeros((repeats))
            @jit()
            def checking_random_walks():
        ...
        trap_time = checking_random_walks()
        ax = plt.axes()
        ax.set_ylabel("$\Phi(t)$")
        ax.set_title(
            f"Plot of Probability of survival $\Phi(t)$ - t for c={density:0.1e}"
        )
        ax.set_xlabel("t")
        max_xaxis_value = int(trap_time.max())
        ...
        plt.plot(
            ...
            survivability_array,
            alpha=0.8,
            label="Probability of survival $\Phi(t)$",
        )
        with np.errstate(
            ...
            y_analytical = pow(
                ...
                * sum(
                    c[j] / np.log(8 * x_analytical_linspace) ** j
    ...
    ax.set_ylabel("$\Phi(t)$")
    ax.set_xlabel("t")
    ax.set_title("Plot of Probability of survival $\Phi(t)$ - t")
    ax.legend(loc="best", fancybox=True, framealpha=1, borderpad=1)
    plt.tight_layout()
    savim("images", "prob1_task6_lin_comb_plot")
    fig = plt.figure()
    ax = plt.axes()
    plt.plot(
        xaxis_for_survivability_array1,
    ...
    ax.set_ylabel("log$\Phi(t)$")
    ax.set_title("Plot of Probability of survival log$\Phi(t)$ - logt")
    ax.legend(loc="best", fancybox=True, framealpha=1, borderpad=1)
    ax.set_yscale("log")
    ax.set_xscale("log")
    ax.set_xlabel("logt")
    plt.tight_layout()
    savim("images", "prob1_task6_log_comb_plot")
